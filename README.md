# NWSCAN — монитор сети для Raspberry Pi v1.1 (Stable)

NWSCAN — легкий монитор сети, который:
- отслеживает наличие IP и доступность интернета (с привязкой к интерфейсам);
...
## Новое в версии 1.1
- **Надежная настройка IP**: Новая стратегия определения NetworkManager vs dhcpcd для предотвращения конфликтов при настройке IP.
- **Умный LED**: Светодиод теперь мигает, если IP получен, но интернета нет (ранее гас). Строгая привязка проверки интернета к интерфейсу исключает ложные срабатывания.
- **Группировка DNS**: В GUI DNS-серверы теперь отображаются с привязкой к конкретным интерфейсам.
- **Улучшенная диагностика**: Команды настройки IP/MAC восстановлены и работают стабильно.

## Возможности
- LED-индикация: 
  - **Выключен**: Нет IP-адреса на мониторимых интерфейсах.
  - **Мигает**: Есть IP, но нет интернета (интервал 0.15 с).
  - **Включен**: Интернет доступен.
...
### Настройка IP и MAC (Новое в v1.1)
- `/set_ip_eth0 dhcp` — включить DHCP на eth0.
- `/set_ip_eth0 <ip> <mask> <gw> [dns]` — установить статический IP на eth0.
- `/set_ip_wlan0 ...` — аналогично для wlan0.
- `/set_mac_eth0 <mac>` — сменить MAC-адрес eth0.
- `/set_mac_eth0 restore` — восстановить заводской MAC.
